...
public class TranslateServiceImpl implements TranslateService {
    ...
    public Translate findById(Long id) {
        TranslateEntity created = translateRepository.findById(id)
                .orElseThrow(() -> new EntityNotFoundException(String.format("translate not found (id=%)", id)));
        return translateMapper.toDto(created);
    }

    @Override
    public Translate createTranslate(Translate translate) {
        TranslateEntity created = translateRepository.save(translateMapper.toEntity(translate));
        return translateMapper.toDto(created);
    }
